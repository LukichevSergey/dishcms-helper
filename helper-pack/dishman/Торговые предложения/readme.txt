...
-----------------------------------
2. во вью \protected\views\shop\_products.php делаем вывод данных и передачу их в корзину

2.1 получаем все активные торговые предложения

$offers = $data->dataAttributeBehavior->get(true);


2.2 переопределяем стандартную цену 

if ($offers) $data->price = $offers[1]['price'];


2.3 добавляем верстку переключателей цены и размера порции 
...
2.4 для кнопки добавления в корзину в атрибуты карты товара добавить еще один атрибут
...
3.1 добвить public $offer;

3.2 добавить поведение 
...
3.3 в scopes в select добавить t.data
...
----------------------------------
4 в \protected\modules\admin\views\shop\ добавить файл _form_product_offers.php - это отображение в админке
...
------------------------------------
5. в \protected\config\dcart.php сделать по аналогии с offer
