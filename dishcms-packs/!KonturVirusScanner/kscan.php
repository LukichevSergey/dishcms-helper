...
class KScan
{
	...
	const QUARANTINE_EXT='.ksuspected';
	
	// @var integer показать правила для файлов, по которым файл 
	// отмечен как подозрительный. 1 - показать, 0 - не показывать.
	const SHOW_RULES = 1;
	
	private $quarantine_exts=['suspected', 'ksuspected'];
	...
	private function prepare()
	{
		...
		elseif(($this->step===self::STEP_SCANING) && $this->cget('filelist_complite', 0)) {
			...
			elseif($result===false) {
				$endtime=time();
				$msg.='<b>Просканировано:</b> ' . ((int)$this->cget('scan_pos', 0)) . ' из ' . static::$cache['filelist_count'] . '<br/>';
    ...
    private function controls()
    {
		...
		if(!empty($virusfiles)) {
			...
			$(document).ready(function(){
				...
				function onBtnToQuarantineClick(e){
					var files=getCheckedDataFileNames($(".checkbox [type='checkbox']:checked"));
					if(confirm("Подтвердите перемещение отмеченных файлов в карантин\n"+files.join("\n"))) {
						$.post("?quarantine&<?=time()?>", {files: getCheckedValues($(".checkbox [type='checkbox']:checked"))}, function(response){
							alert(response);
							window.location.reload();
						});
					}
				}
				function onBtnShowRulesClick(e){}
				...
				$(document).on("click", ".btn-to-quarantine", onBtnToQuarantineClick);
			...
			<div id="kscan_controls">
				<div class="panel">
					...
					<div class="filter" style="float:right;margin-left:30px;background:#FFC107;padding: 10px;">
						...
						<div class="filter" style="float:right;margin-left:30px;background:#FFC107;padding: 0;">
							<button class="btn-to-quarantine" style="color: #2196f3;">Карантин</button>
						</div>
					</div>
				</div>
				...
				foreach($virusfiles as $filename) {
					...
					$isfile=is_file($filename);$isquarantine=is_file($filename . self::QUARANTINE_EXT) || preg_match('#\.('.implode('|',$this->quarantine_exts).')$#', $filename);
					?><tr<?=($isfile&&$verifed&&!$isquarantine)?' class="preverifed"':''?><?=$isquarantine?' class="quarantine"':($isfile?'':' class="deleted"')?>>
						<td class="checkbox<?=$isquarantine?' quarantine':($isfile?'':' deleted')?>">
							<? if($isfile): ?><input type="checkbox" data-filename="<?=preg_replace('#^'.dirname(__FILE__).'#', '', $filename)?>" value="<?=$filename?>"/><? else: ?><?=$isquarantine?'карантин':'удален'?><? endif; ?>
						</td>
						<td><? if($isfile): ?><a data-fancybox href="?view=<?=$filename?>&<?=time()?>">просмотр</a>
						<? elseif($isquarantine&&!$isfile): ?><a data-fancybox href="?view=<?=$filename.self::QUARANTINE_EXT?>&<?=time()?>">просмотр</a>
						<? else: ?>&nbsp;<? endif; ?></td>
						...
						<td><?if(self::SHOW_RULES):?><?$this->printVirusInfo($filename.(is_file($filename . self::QUARANTINE_EXT)?self::QUARANTINE_EXT:''), true);?><?else:?>
							<?=preg_replace('#^'.dirname(__FILE__).'#', '', $filename)?><?if($isquarantine && !$isfile)echo self::QUARANTINE_EXT;?>
						<?endif;?></td>
					</tr><?
				}
			?></tbody></table><?
    ...
    {
    	$isquarantine=false;
    	if(preg_match('#\\'.self::QUARANTINE_EXT.'$#', $filename)) $isquarantine=true;
    	if(is_file($filename) && ($isquarantine || $this->existInLog($filename, $this->getVirusFileListFilename()))) {
    		echo '<div>';
    ...
    private function printVirusInfo($filename, $onlyRules=false)
    {
    	$founds=$this->search($filename);
		if(is_array($founds)) {
			?>
			<style>.view-info {
			    ...
			    color: #333;
			    padding: 10px;
			    width: <?=$onlyRules?'100%':'80%';?>;;
			    margin: 0 auto<?if(!$onlyRules)echo' 20px';?>;
			    border-radius: 5px;
			    font-size: 14px;
			}
			.view-info-header {
    ...
    private function quarantine()
    {
    	...
    	if(array_key_exists('files', $_POST)) {
    		if(is_array($_POST['files'])) {
    			foreach($_POST['files'] as $filename) {
    				if(is_file($filename) && $this->existInLog($filename, $this->getVirusFileListFilename())) {
    					$quarantineFilename=$filename . self::QUARANTINE_EXT;
    					@rename($filename, $quarantineFilename);
    					if(!is_file($quarantineFilename)) {
    						$errors[]='ОШИБКА! ФАЙЛ НЕ БЫЛ ПЕРЕМЕЩЕН В КАРАНТИН! ' . $filename;
    					}
    					else {
    						$quarantine[]=$filename;
    					}
    			...
    			$errors[]='Не найдено файлов для перемещения в карантин.';
    		}
    	}
    	else {
    		$errors[]='Не найдено файлов для перемещения в карантин.';
    	...
    	if($errors) {
    		echo 'При переносе файлов в карантин возникли ошибки!';
    		echo "\n" . implode("\n", $errors) . "\n";
    	}
    	if($deleted) {
    		echo 'Файлы успешно перенесены в карантин!';
    		echo "\n" . implode("\n", $quarantine);
    	}
    }
