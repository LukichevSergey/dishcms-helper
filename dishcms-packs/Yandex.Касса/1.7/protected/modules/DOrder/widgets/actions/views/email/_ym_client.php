...
?>
<p>Здравствуйте, <?php echo $customer['CustName']['value']; ?></p>
<p>Заказ <?php echo '№'. $model->id; ?></p>

<?php 
foreach($customer as $name=>$data): 
	if(in_array($name, array('orderNumber', 'shopSuccessURL', 'shopFailURL', 'CustEMail', 'scid', 'Sum', 'ShopID', 'CustomerNumber', 'CustName')) || empty($data['value'])) continue;
	if($name == 'paymentType') $data['value'] = \DOrder\models\YandexForm::model()->getPaymentTypeLabel($data['value']); 
	?>
	<p>
	    <strong><?php echo ($name == 'CustName') ? 'Имя' : $data['label']; ?></strong>:<br />
...
<?php if($customer['paymentType']['value'] == 'Handsup'):?>
