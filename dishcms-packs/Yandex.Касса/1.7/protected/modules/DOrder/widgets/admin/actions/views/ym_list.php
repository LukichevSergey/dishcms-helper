    ...
    <?php foreach($model as $item): ?>
        <tr class="order<?php if($item->paid == 1) echo ' payment_complete'; ?> dorder-list-item" data-item="<?php echo $item->id; ?>">
            ...
            <td colspan="2">
            	<?php $customer = $item->getCustomerData(); ?>
           		<?php if(isset($customer['paymentType'])): ?>
                    <?php echo \CHtml::link($customer['CustName']['value'], 'javascript:void()', array('class' => 'orderuser', 'data-item' => $item->id)); ?>,
   	                <?php echo $customer['CustEMail']['value']; ?>,
       	            <?php echo $customer['phone']['value']; ?>,
           	        <?php echo $customer['CustAddr']['value']; ?>.
             	    <br/><i><?php echo $customer['paymentType']['label']; ?>:</i> <?php echo \DOrder\models\YandexForm::model()->getPaymentTypeLabel($customer['paymentType']['value']); ?>
                <?php else: ?>
        ...
        <?php endforeach; ?>
            <tr class="details dorder-list-item-comment" data-item="<?php echo $item->id; ?>">
                <td colspan="7"><textarea data-item="<?php echo $item->id; ?>" class="comment"><?php echo $item->comment ?: @$customer['OrderDetails']['value']; ?></textarea></td>
            </tr>
    <?php endforeach; ?>
</table>
<br />
