        ...
        formData: {hash: "<?=$this->hash?>"},
            ...
            if(data.result.error==1){
                ...
                if("jpg,jpeg,gif,png".indexOf(data.result.ext) > -1) {
                    $('.form-upload_<?=$this->hash?> .image_place').append('<a href="javascript:;" data-src="'+data.result.img+'" data-fancybox="1" rel="form-<?= $this->id; ?>"><i data-src="'+data.result.img+'" class="js-file-delete fas fa-times-circle" title="Удалить">удалить</i><img src="'+data.result.img+'"></a>');
                }
                else {
                    $('.form-upload_<?=$this->hash?> .image_place').append('<a href="javascript:;"><i data-src="'+data.result.img+'" class="js-file-delete fas fa-times-circle" title="Удалить">удалить</i><i class="filename fas fa-file-alt" title="'+data.result.origin+'">'+data.result.origin+'</i></a>');
                }
            }
...
.form-upload .image_place { width: 100%; }
.form-upload .image_place a{display:inline-block;margin:2px;vertical-align:top;position:relative;}
.form-upload .image_place a img{max-width:100%;max-height:250px;}
.form-upload .image_place a i.js-file-delete{background:#fa69b6;font-style:normal;position: absolute;right: 0;color: #fff;border: 1px solid #fff;font-size: 13px;padding: 2px 7px;}
.form-upload .image_place a i.js-file-delete:hover{cursor:pointer;background:#e764c2;}	
