...
class HYKassaHistory
{
     ...
     * 
     * @return \crud\models\ar\ykassa\models\History|false
     */
    public static function create($params, $comment='', $configurationId=null)
    {
        $history=new History;
        $history->configuration_id=(string)$configurationId;
        $history->uuid=HHash::guid();
        $history->status=HYKassa::API_STATUS_NEW;
     ...
     * Получить URL оплаты
     * 
     * @param int|\crud\models\ar\ykassa\models\History $payment идентификатор платежа, 
     * либо объект платежа.
     * @return string|false
     */
    public static function getConfirmationUrl($payment)
    {
    	$id=($payment instanceof History) ? $payment->id : $payment;
    	if($extra=PaymentExtra::model()->findByAttributes(['payment_id'=>$id])) {
            if($extra->confirmation_url) {
            	return $extra->confirmation_url;
            }
    	}
    ...
    /**
     * Запись URL оплаты в параметры платежа
     *
     * @param \YandexCheckout\Model\Payment $payment объект платежа
     * @return bool
     */
    public static function setConfirmationUrl($payment)
    {
        if($confirmation=$payment->getConfirmation()) {
            if($confirmationUrl=$confirmation->getConfirmationUrl()) {
                if($ypayment=History::model()->findByAttributes(['payment_id'=>$payment->getId()], ['order'=>'`create_time` DESC'])) {
                    if($extra=PaymentExtra::model()->findByAttributes(['payment_id'=>$ypayment->id])) {
                        if($extra->confirmation_url !== $confirmationUrl) {
                            $extra->confirmation_url=$confirmationUrl;
                            return (bool)$extra->update(['confirmation_url']);
                        }
                        else {
                            return true;
                        }
                        ...
                        $extra=new PaymentExtra;
                        $extra->payment_id=$ypayment->id;
                        $extra->confirmation_url=$confirmationUrl;
                        return $extra->save();
                    }
                }
