...
return function(&$event) {
    if($advert=$event->getParam('advert')) {
        if(\D::cms('email') && filter_var(\D::cms('email'), FILTER_VALIDATE_EMAIL)) {
            ...
            $sended=HAccountEmail::sendMail(\D::cms('email'), 'admin_advert_edited', compact('advert', 'account'));
            
            AdvertEmail::add($advert->id, 'admin_advert_edited', compact('advert', 'account'), $sended, \D::cms('email'));
        }
    }
};