...
class AuthController extends \accounts\components\base\BaseController
{
    ...
    public function accessRules()
    {
        return [
            ...
            ['allow', 'users'=>['@'], 'roles'=>[Account::ROLE_RETAIL_BUYER, Account::ROLE_WHOLESALE_BUYER], 'actions'=>['logout']],
            ['deny', 'users'=>['*']]
        ];
    }
            ...
            if($account->validate()) {
                if($acc=$account->byEmail($account->email)->find()) {
                    if($acc->isWaitModeration()) {
                        $account->addError('email', 'Ваш аккаунт находится на модерации.');
                    }
                    elseif(!$acc->published) {
                        $account->addError('email', 'Ваш аккаунт временно заблокирован.');
                    }
                    /*
                    if(!$acc->published) {
                    ...
                    */
                        ...
                        if($acc->regenerateCodes(true)) {
                            ...
                            $account->addError('email', 'Не удалось отправить письмо с ссылкой восстановления пароля. Пожалуйста, обратитесь в службу технической поддержки.');
                        }
                    }
                }
                else {
