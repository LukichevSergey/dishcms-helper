...
return [
    ...
    'menu'=>[
        'backend'=>['label'=>'История платежей']
    ],
    'buttons'=>[
        'create'=>['label'=>''],
        'custom'=>function() {
            ...
            'onBeforeLoad'=>function() use (&$historyDataProvider) {
                ...
                $historyDataProvider=[
                    'criteria'=>[
                        'select'=>'t.*',
                        'group'=>'`t`.`payment_id`',
                        'join'=>'INNER JOIN `reg` ON (`reg`.`id`=`t`.`int_param_1`)',
                        'condition'=>'`reg`.`region_id`=:regionId',
                        'params'=>['regionId'=>Region::getCurrentRegionId()]
                    ],
                        ...
                        'value'=>function($data) {
                            ...
                            Y::js('payment-detail', '$(document).on("click", ".js-payment-detail", function(e) {
                                ...
                                if(detailBox.length>0) {
                                    ...
                                    $.post("/cp/crud/index?cid=ykassa_history", {showdetail: 1, id: paymentId}, function(html) {
