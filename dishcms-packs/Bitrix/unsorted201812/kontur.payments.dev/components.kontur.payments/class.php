...
class KonturPaymentsComponent extends CBitrixComponent
{
	...
	public static function ypayment($arParams, $arData)
	{
	    ...
	    if(!empty($arParams['IBLOCK_ID']) && !empty($arData['amount'])) 
	    {
	        ...
	        $arFields['PROPERTY_VALUES']['CREDITORS_FOR_DOC']=[];
	        $arFields['PROPERTY_VALUES']['CREDITORS_ADDRESS_FOR_DOC']=[];
   	        $arFields['PROPERTY_VALUES']['CREDITORS_NAMES_FOR_DOC']=[];
	        foreach($arData['creditors'] as $creditor) {
	            ...
	            $arFields['PROPERTY_VALUES']['CREDITORS_NAMES_FOR_DOC'][] = $creditor['creditor_name'];
	            $arFields['PROPERTY_VALUES']['CREDITORS_ADDRESS_FOR_DOC'][] = $creditor['creditor_address'];
	...
	public static function getOrder($arFilter)
	{
	    ...
	    if($order=$rs->Fetch()) {
	        ...
	        while($prop=$rs->Fetch()) {
	        	if(in_array($prop['CODE'], ['CREDITORS_FOR_DOC', 'CREDITORS', 'CREDITORS_NAMES_FOR_DOC', 'CREDITORS_ADDRESS_FOR_DOC', 'DISCLAIMER_FILES'])) {
	...
	public static function updateStatus($arParams, $orderId, $paymentStatusValueId, $isPaymentId=false, $sendDisclaimerDoc=false)
	{
	    ...
	    if($order=static::getOrder($arFilter)) {
	        ...
	        if($sendDisclaimerDoc) {
	            ...
	            $rs=\CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $order['ID'], ['NAME'=>'ASC'], ['CODE'=>'DISCLAIMER_FILES']);
	            while($file=$rs->Fetch()) {
		            $arFiles[]=$file['VALUE'];
		        }
	...
	{
		$templateFilenameDoc = dirname(__FILE__) . '/docs/disclaimer_doc_tpl_doc.docx';
		$templateFilenameHelp = dirname(__FILE__) . '/docs/disclaimer_doc_tpl_help.docx';
		...
		$baseOrderDir=$_SERVER['DOCUMENT_ROOT'].'/upload/disclaimers/' . md5($order['ID']);
		foreach($order['PROPERTIES']['CREDITORS_FOR_DOC'] as $idx=>$creditor) {
			$baseDir=$baseOrderDir . '/' . ($idx + 1);
			if(!is_dir($baseDir)) {
				mkdir($baseDir, 0755, true);
		        chmod($baseDir, 0755);
			}
			$outputFilenameDoc = $baseDir . '/Заявление об отказе.docx';
			$outputFilenameHelp = $baseDir . '/Инструкция.docx';
			
			$arFields = [
				'CREDITOR_NAME'=>$order['PROPERTIES']['CREDITORS_NAMES_FOR_DOC'][$idx]['VALUE'],
				'CREDITOR_ADDRESS'=>$order['PROPERTIES']['CREDITORS_ADDRESS_FOR_DOC'][$idx]['VALUE'],
			...
			$rs=\CIBlockElement::GetPropertyValues($order['IBLOCK_ID'], ['ID'=>$order['ID']], false, ['CODE'=>'DISCLAIMER_FILES']);
			while($ar=$rs->Fetch()) {
				$arFiles[]=\CFile::GetFileArray($ar['VALUE']);
			}
			
			static::attachDoc($templateFilenameDoc, $outputFilenameDoc, $arFields);		
			$arFiles[]=CFile::MakeFileArray($outputFilenameDoc);
			
			$outputFilenameHelpPDF=static::attachDoc($templateFilenameHelp, $outputFilenameHelp, $arFields, true);		
			$arFiles[]=CFile::MakeFileArray($outputFilenameHelpPDF);
			
		
			\CIBlockElement::SetPropertyValueCode($order['ID'], 'DISCLAIMER_FILES', $arFiles);
			
			if(is_file($outputFilenameDoc)) unlink($outputFilenameDoc);
			if(is_file($outputFilenameHelp)) unlink($outputFilenameHelp);
			if(is_file($outputFilenameHelpPDF)) unlink($outputFilenameHelpPDF);			
			@rmdir($baseDir);
		}
		@rmdir($baseOrderDir);		
	...
	protected static function attachDoc($templateFilename, $outputFilename, $values=[], $isPDF=false)
	{
...
//		file_put_contents(dirname(__FILE__).'/log.log', $outputFilename."\n\n", FILE_APPEND);
		...
		if($isPDF) {
			...
			$rendererLibraryPath = dirname(__FILE__) . '/vendors/TCPDF/';
			\PhpOffice\PhpWord\Settings::setPdfRenderer($rendererName, $rendererLibraryPath);
		
			$xmlWriter = \PhpOffice\PhpWord\IOFactory::createWriter($PHPWord , 'PDF');
			$outputFilenamePDF=preg_replace('/\.docx$/', '.pdf', $outputFilename);
			$xmlWriter->save($outputFilenamePDF);			
		
			@unlink($outputFilename);
			$outputFilename = $outputFilenamePDF;
		}
