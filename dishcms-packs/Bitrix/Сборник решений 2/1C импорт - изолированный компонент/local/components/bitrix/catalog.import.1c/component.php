...
require_once dirname(__FILE__) . '/classes/general/cml2.php';
require_once dirname(__FILE__) . '/classes/mysql/cml2.php';
__IncludeLang(dirname(__FILE__).'/lang/ru/cml2_utf8.php');

error_reporting(E_ALL & ~E_NOTICE & ~E_STRICT & ~E_DEPRECATED & ~E_WARNING);

// функция отладки
if( !function_exists('log1c') ) {
	function log1c() {
		$args = func_get_args();
		if(!empty($args[0]) && is_numeric($args[0])) { $n=$args[0]; unset($args[0]); }
		else $n=0;
		file_put_contents(dirname(__FILE__)."/debug_{$n}_".date('d_m_Y_H_i_s').".log", var_export($args, true) . "\n\n--------------------------------\n\n", FILE_APPEND);
	}
}
...
elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"])
{
	...
	$result = CIBlockXMLFileCustom::UnZip($_SESSION["BX_CML2_IMPORT"]["zip"], $_SESSION["BX_CML2_IMPORT"]["last_zip_entry"]);
	if ($result===false)
	{
...
try {
	if ($NS["STEP"] < 1)
	{
		CIBlockXMLFileCustom::DropTemporaryTables();
		$strMessage = GetMessage("CC_BSC1_TABLES_DROPPED");
		$NS["STEP"] = 1;
	}
	elseif ($NS["STEP"] == 1)
	{
		if (CIBlockXMLFileCustom::CreateTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 2)
	{
		...
		{
			$obXMLFile = new CIBlockXMLFileCustom;
	...
	elseif ($NS["STEP"] == 7)
	{
		...
		{
			if (CIBlockXMLFileCustom::IsExistTemporaryTable())
			{
				$NS["DONE"]["ALL"] = CIBlockXMLFileCustom::GetCountItemsWithParent($NS["XML_ELEMENTS_PARENT"]);
			}
			else
			{
