...
if(!Loader::includeModule("kontur.salestat")) {
	ShowError('Module kontur.salestat not loaded');
}
...
if(!empty($productIDs)) {
	...
	$rs=\CIblockElement::GetList([],['=ID'=>$productIDs, 'IBLOCK_ID'=>Helper::getOffersIblockId()], false, false, [
		'ID', 'IBLOCK_ID', 'PROPERTY_CML2_LINK', 'PROPERTY_COLOR', 'PROPERTY_SIZE_HEIGHT', 'PROPERTY_SIZE_CLOTHES', 'PROPERTY_VOLUME'
	]);
	while($el=$rs->Fetch()) {
		if(!empty($el['PROPERTY_CML2_LINK_VALUE'])) {
			$productIDs[$el['ID']]=$el['PROPERTY_CML2_LINK_VALUE'];
			$props[$el['ID']]=[
				'COLOR'=>['LABEL'=>'цвет', 'VALUE'=>$el['PROPERTY_COLOR_VALUE']],
				'SIZE_HEIGHT'=>['LABEL'=>'рост', 'VALUE'=>$el['PROPERTY_SIZE_HEIGHT_VALUE']],
				'SIZE_CLOTHES'=>['LABEL'=>'размер', 'VALUE'=>$el['PROPERTY_SIZE_CLOTHES_VALUE']],
				'VOLUME'=>['LABEL'=>'объем', 'VALUE'=>$el['PROPERTY_VOLUME_VALUE']]
			];
		}
	}
	
	if(!empty($productIDs)) {
		$rs=\CIblockElement::GetList([],['=ID'=>$productIDs, 'IBLOCK_ID'=>Helper::getItemsIblockId()], false, false, ['ID', 'IBLOCK_ID', 'PROPERTY_CML2_ARTICLE']);
		while($el=$rs->Fetch()) { 
			$productId=$el['ID'];
			foreach($productIDs as $key=>$prodId) {
				if($prodId == $productId) {
					$arResult[$key]['ARTICLE']=$el['PROPERTY_CML2_ARTICLE_VALUE'];
					if(!empty($props[$key])) {
						$vProps=[];
						foreach($props[$key] as $prop) {
							if(!empty($prop['VALUE'])) {
								$vProps[]=$prop['LABEL'] . ': ' . $prop['VALUE'];
							}
						}
						if(!empty($vProps)) {
							$arResult[$key]['NAME'].=mb_strtolower(' (' . implode(', ', $vProps) . ')');
						}
					}
				}
			}
...
function bxStatSort($a, $b)
{
	...
	if(in_array($by, Array("PRODUCT_ID", "ARTICE", "PAYED", "ALLOW_DELIVERY", "ORDER_COUNT", "COUNT", "QUANTITY", "DELAY", "ORDER_QUANTITY", "BASKET_QUANTITY")))
	{
...
$arHeaders = array(
	...
	array("id"=>"ARTICLE", "content"=>GetMessage("SALE_ARTICLE"), "sort"=>'ARTICLE', "default"=>true),
...
while ($arResult = $dbResult->GetNext())
{
	...
	$row->AddViewField("ARTICLE", $arResult["ARTICLE"]??'-');
