...
class CIBlockCMLImportWS
{
	...
	/** @var CIBlockXMLFileWS $_xml_file */
	var $_xml_file = null;
	...
	function initMess($return=false)
	{
	    // $this->mess = IncludeModuleLangFile(__FILE__, $this->next_step["lang"], true);
	    $this->mess = __IncludeLang(dirname(__FILE__).'/../../lang/ru/cml2ws_cp1251.php', true);
	...
	function Init(&$next_step, $files_dir = false, $use_crc = true, $preview = false, $detail = false, $use_offers = false, $use_iblock_type_id = false, $table_name = "b_xml_tree")
	{
		...
		$this->_xml_file = new CIBlockXMLFileWS($table_name);
...
//			$arIBlock['CODE']=CUtil::translit($arIBlock['NAME'], 'ru', array('change_case'=>'L', 'replace_space'=>'-'));
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		$rsElement = $obElement->GetList(
			...
			false, false,
			Array("ID", "TMP_ID", "ACTIVE", "CODE", "PREVIEW_PICTURE", "DETAIL_PICTURE", "IBLOCK_SECTION_ID")
		);
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if($arDBElement)
			{
				...
				if ( isset($arElement['IBLOCK_SECTION']) && isset($arDBElement['IBLOCK_SECTION_ID']) && !empty($arDBElement['IBLOCK_SECTION_ID']) ) {
				    unset($arElement['IBLOCK_SECTION']);
				}
