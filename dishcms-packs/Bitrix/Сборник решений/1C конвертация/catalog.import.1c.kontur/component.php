...
include(dirname(__FILE__).'/classes/general/cml2.php');
include(dirname(__FILE__).'/classes/mysql/cml2.php');
__IncludeLang(dirname(__FILE__).'/lang/ru/cml2ws_cp1251.php');
...
elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"])
{
	...
	$result = CIBlockXMLFileWS::UnZip($_SESSION["BX_CML2_IMPORT"]["zip"], $_SESSION["BX_CML2_IMPORT"]["last_zip_entry"]);
	if ($result===false)
	{
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	if ($NS["STEP"] < 1)
	{
	    CIBlockXMLFileWS::DropTemporaryTables();
		$strMessage = GetMessage("CC_BSC1_TABLES_DROPPED");
		$NS["STEP"] = 1;
	}
	elseif ($NS["STEP"] == 1)
	{
	    if (CIBlockXMLFileWS::CreateTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 2)
	{
		...
		{
		    $obXMLFile = new CIBlockXMLFileWS;
	...
	elseif ($NS["STEP"] == 3)
	{
	    if (CIBlockXMLFileWS::IndexTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 4)
	{
		$obCatalog = new CIBlockCMLImportWS;
	...
	elseif ($NS["STEP"] == 7)
	{
		...
		{
		    if (CIBlockXMLFileWS::IsExistTemporaryTable())
			{
			    $NS["DONE"]["ALL"] = CIBlockXMLFileWS::GetCountItemsWithParent($NS["XML_ELEMENTS_PARENT"]);
			}
			else
			{
