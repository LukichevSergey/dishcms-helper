...
class CIBlockCMLCustomImport
{
	...
	function ImportSections()
	{
		if($this->next_step["XML_SECTIONS_PARENT"])
		{
			...
			foreach($arID as $id)
			{				
				$result = $this->ImportSectionCustom($id, $this->next_step["IBLOCK_ID"], false);
				if($result !== true)
					return $result;
			}
		}
		...
		if($this->next_step["XML_ELEMENTS_PARENT"])
		{
			...
			while($arParent = $rsParents->Fetch())
			{
				...
				elseif(array_key_exists($this->mess["IBLOCK_XML2_BX_TAGS"], $arXMLElement))
				{
					...
					$ID = $this->ImportElementCustom($arXMLElement, $counter, $bWF, $arParent);
				}
				else
				{
	...
	function CheckIfElementIsActive($arXMLElement)
	{
		...
		return $bActive;
											...
											function ImportElementCustom($arXMLElement, &$counter, $bWF, $arParent)
											{
												...
												elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
												{
													...
													if($arDBElement)
													{
														...
														)
														{
															unset($arElement["DETAIL_PICTURE"]);
														 						...
														 						/*    CUSTOM IMPORT    */
											                                        $arSelect = Array("ID", "NAME", "IBLOCK_SECTION_ID", "XML_ID");
											                                        $arFilter = Array("IBLOCK_ID"=>IntVal($this->next_step["IBLOCK_ID"]), "XML_ID" => $arElement["XML_ID"]);
											                                        ...
											                                        if($arFields["IBLOCK_SECTION_ID"]){
											                                            $arElement["IBLOCK_SECTION"][0] = $arFields["IBLOCK_SECTION_ID"];
											                                            $arElement["IBLOCK_SECTION_ID"] = $arFields["IBLOCK_SECTION_ID"];
											                                            $arElement["NAME"] = $arFields["NAME"];
											                                        }
										                                        /*****************************/
										                                        /*?><pre><?print_r($arElement)?></pre><?die();*/
													...
													else
													{
														...
														$this->fillDefaultPropertyValues($arElement, $this->arProperties);
																			...
																			/*  CUSTOM IMPORT   */
										                                        /*$newProductSection = '';
										                                        $arFilter = array('IBLOCK_ID' => $arElement["IBLOCK_ID"], "ACTIVE" => "N", 'DEPTH_LEVEL' => 1);
										                                        $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'), $arFilter, false, array("ID", "NAME"));
										                                        while ($arSect = $rsSect->GetNext()) {
										                                            $arFirstLevel[$arSect["ID"]] = $arSect["NAME"];
										                                        }

										                                        if(!in_array(GetMessage("IBLOCK_XML2_NEW_PRODUCTS"), $arFirstLevel)){
										                                            $iBlockSect = new CIBlockSection;

										                                            $arFieldsFirstSect = Array(
										                                                "ACTIVE" => "N",
										                                                "NAME" => GetMessage("IBLOCK_XML2_NEW_PRODUCTS"),
										                                                "CODE" =>  Cutil::translit(GetMessage("IBLOCK_XML2_NEW_PRODUCTS"), "ru"),
										                                                "IBLOCK_ID" => $arElement["IBLOCK_ID"],
										                                                "SORT" => 50

										                                            );
										                                            $newProductSection = $iBlockSect->Add($arFieldsFirstSect);
										                                        }else{
										                                            foreach($arFirstLevel as $key => $value){
										                                                if($value == GetMessage("IBLOCK_XML2_NEW_PRODUCTS")){
										                                                    $newProductSection = $key;
										                                                }
										                                            }
										                                        }

										                                        if($newProductSection){
										                                            $arElement["IBLOCK_SECTION"][0] = $newProductSection;
										                                            $arElement["IBLOCK_SECTION_ID"] = $newProductSection;
										                                            $arElement["ACTIVE"] = "N";
										                                        }*/
										                                    /*******************/
	...
	function ImportSectionCustom($xml_tree_id, $IBLOCK_ID, $parent_section_id)
	{
					...
					if($arField1["USER_TYPE"]["BASE_TYPE"] == "file")
					{
						...
						break;
									...
									/*    CUSTOM IMPORT    */
                                            $trumb = [];
                                            $custom_name = '';
                                            $arFilter = array('IBLOCK_ID' => $IBLOCK_ID, '=XML_ID' => $arSection["XML_ID"]);
                                            $rsSections1 = CIBlockSection::GetList(array('ID' => 'ASC'), $arFilter);
                                            while ($arNewSection = $rsSections1->Fetch())
                                            {
                                                $trumb = $arNewSection["IBLOCK_SECTION_ID"];
                                                $custom_name = $arNewSection["NAME"];
                                                $custom_code = $arNewSection["CODE"];
                                            }
                                            if($trumb && !empty($trumb)) {
                                                $arSection["IBLOCK_SECTION_ID"] = $trumb;
                                                $arSection["NAME"] = $custom_name;
                                                $arSection["CODE"] = $custom_code;
                                            }
                                    /************************/
		...
		if($XML_SECTIONS_PARENT)
		{
			...
			while($ar = $rs->Fetch())
			{				
				$result = $this->ImportSectionCustom($ar["ID"], $IBLOCK_ID, $arSection["ID"]);
				if($result !== true)
					return $result;
			}
		}
						...
						break;
									...
									/*    CUSTOM IMPORT    */
                                            ...
                                            if($trumb && !empty($trumb)) {
                                                ...
                                                $arSection["IBLOCK_SECTION_ID"] = 280;
                                                //$arSection["NAME"] = $custom_name;
                                            }*/
                                    /************************/
