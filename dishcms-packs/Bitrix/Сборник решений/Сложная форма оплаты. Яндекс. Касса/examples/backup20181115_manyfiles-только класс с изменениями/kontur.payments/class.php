...
class KonturPaymentsComponent extends CBitrixComponent
{
	...
	public static function getOrder($arFilter)
	{
	    ...
	    if($order=$rs->Fetch()) {
	        ...
	        while($prop=$rs->Fetch()) {
	        	if(in_array($prop['CODE'], ['CREDITORS_FOR_DOC', 'CREDITORS', 'CREDITORS_NAMES_FOR_DOC', 'DISCLAIMER_FILES'])) {
	...
	protected static function attachDisclaimerDoc($order)
	{
		...
		foreach($order['PROPERTIES']['CREDITORS_FOR_DOC'] as $idx=>$creditor) {
			$outputFilename = $_SERVER['DOCUMENT_ROOT'].'/upload/disclaimers/' . $order['ID'] . '_' . ($idx+1) . '.docx';
			$arFields = [
