...
class KonturPaymentsComponent extends CBitrixComponent
{
    ...
    const SBR_LOGIN='geliosdent-api';
    const SBR_PASSWORD='geliosdent';
    const SBR_MERCHANT='geliosdent';
	...
	protected static function updateStatus($arParams, $orderId, $paymentStatusValueId, $isPaymentId=false)
	{
	    ...
	    if($order=$rs->Fetch()) {
	        ...
	        static::send($arParams, $arEventFields);
	    }
	}
	
	protected static function send($arParams, $arFields)
	{
	    if(!empty($arParams['EVENT_TYPE']) && !empty($arParams['EVENT_ID'])) {
            \CEvent::Send($arParams['EVENT_TYPE'], SITE_ID, $arFields, 'Y', $arParams['EVENT_ID']);
	    }
	}
