...
$fC=function($text, $bHSC=true) use ($fHSC) {
    global $APPLICATION;
...
$fAttrs=function($attrs=array()) {
    if(!empty($attrs)) {
        foreach($attrs as $name=>$v) $s[]="{$name}=\"{$v}\"";
        return ' '.implode(' ', $s);
    }
    return '';
};
...
$fTag=function($tag, $content, $attrs=array(), $convert=true, $bHSC=true) use ($fW, $fC, $fHSC, $fTagO, $fTagC) {
    if($convert) $content=$fC($content, $bHSC);
...
if(empty($arRunErrors)) { // offers
    ...
    $rsProducts=CIBlockElement::GetList(
        ...
        false, 
        false,
        array('IBLOCK_ID', 'ID', 'IBLOCK_SECTION_ID', 'NAME', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'PROPERTY_OLD_PRICE')
    );
    $arProducts=array();
    $arProductIDs=array();
    ...
    if(!empty($arProducts)) {
        $fOfferTag=function($arOffer, $arProps=array(), $isOffer=false, $arProduct=null) 
            use ($ar_iblock, $XML_DATA, $usedProtocol, $iblockServerName, $fHSC, $fTagO, $fTag, $fTagC)
        {
            $minPrice = 0;
			if ($XML_DATA['PRICE'] > 0) {
...
//                $fTag('id', $arOffer['ID']);
                ...
                $fTag('price', $minPrice);
                if(!empty($arProps['OLD_PRICE']['VALUE']) && ((int)$minPrice < (int)$arProps['OLD_PRICE']['VALUE'])) {
                    $fTag('oldprice', $arProps['OLD_PRICE']['VALUE']);
                }
                $fTag('currencyId', 'RUB');
				if($isOffer) $fTag('categoryId', $arProduct['IBLOCK_SECTION_ID']);
				else $fTag('categoryId', $arOffer['IBLOCK_SECTION_ID']);
                
                $DETAIL_PICTURE = (int)$arOffer["DETAIL_PICTURE"];
                $PREVIEW_PICTURE = (int)$arOffer["PREVIEW_PICTURE"];
                if($DETAIL_PICTURE > 0 || $PREVIEW_PICTURE > 0) {
                    ...
                    if ($ar_file = CFile::GetFileArray($pictNo)) {
                        if(substr($ar_file["SRC"], 0, 1) == "/")
                            ...
                            $strFile = $ar_file["SRC"];
                        $fTag('picture', $strFile);
                    }
                }

				$fTag('local_delivery_cost', '0');
                
                if($isOffer && !empty($arProps['SPECIFICATION']['VALUE'])) {
                	$SPECIFICATION = ' (' . $arProps['SPECIFICATION']['VALUE'] . ')';
                }
                else $SPECIFICATION='';
                $fTag('name', $arOffer['NAME'] . $SPECIFICATION);
                ...
                $fTag('description', trim($ipvValues['ELEMENT_META_DESCRIPTION'] . $SPECIFICATION));
                  
                $fTag('sales_notes', 'Необходима предоплата в размере 50%');
            $fTagC('offer');
        };
        ...
        foreach($arProducts as $id=>$arProduct) { 
            if($arOffersExists[$id]) {
                	...
                	foreach($arAllOffers as $id=>$arOffers) {
						foreach($arOffers as $arOffer) {
    	            		...
    	            		if($offer=$rs->GetNextElement()) {
								$rsOldPrice=CIBlockElement::GetProperty($arOffer['IBLOCK_ID'], $arOffer['ID'], array(), array('CODE'=>'OLD_PRICE'));
					            $arOldPrice=$rsOldPrice->Fetch();
	    	                	$fOfferTag(
                                    $offer->GetFields(), 
                                    array(
                                        'SPECIFICATION'=>$offer->GetProperty('SPECIFICATION'),
                                        'OLD_PRICE'=>$arOldPrice
                                    ), 
                                    true, 
                                    $arProduct
                                );
				...
				$rsOldPrice=CIBlockElement::GetProperty($arProduct['IBLOCK_ID'], $arProduct['ID'], array(), array('CODE'=>'OLD_PRICE'));
	            $arOldPrice=$rsOldPrice->Fetch();
                $fOfferTag($arProduct, array('OLD_PRICE'=>$arOldPrice));
            }
        } 
    }
