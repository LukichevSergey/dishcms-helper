...
require dirname(__FILE__) . '/classes/iblock/general/cml2.php';
require dirname(__FILE__) . '/classes/iblock/mysql/cml2.php';
...
elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"])
{
	...
	$result = KCIBlockXMLFile::UnZip($_SESSION["BX_CML2_IMPORT"]["zip"], $_SESSION["BX_CML2_IMPORT"]["last_zip_entry"]);
	if ($result===false)
	{
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	if ($NS["STEP"] < 1)
	{
		KCIBlockXMLFile::DropTemporaryTables();
		$strMessage = GetMessage("CC_BSC1_TABLES_DROPPED");
		$NS["STEP"] = 1;
	}
	elseif ($NS["STEP"] == 1)
	{
		if (KCIBlockXMLFile::CreateTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 4)
	{
		$obCatalog = new KCIBlockCMLImport;
	...
	elseif ($NS["STEP"] == 7)
	{
		...
		{
			if (KCIBlockXMLFile::IsExistTemporaryTable())
			{
				$NS["DONE"]["ALL"] = KCIBlockXMLFile::GetCountItemsWithParent($NS["XML_ELEMENTS_PARENT"]);
			}
			else
			{
