...
IncludeModuleLangFile(dirname(__FILE__) . '/.last_version/cml2.php', LANGUAGE_ID);

class KCIBlockCMLImport
{
	var $LAST_ERROR = "";
	var $next_step = false;
	var $files_dir = false;
	...
	var $arMeasureUnits = array();
	
	function initMess($return=false)
    {
    	$this->mess = include(dirname(__FILE__).'/.last_version/lang/ru/_cml2.php');
    	// $this->mess = __IncludeLang(dirname(__FILE__).'/.last_version/cml2.php', true);
	...
	function Init(&$next_step, $files_dir = false, $use_crc = true, $preview = false, $detail = false, $use_offers = false, $use_iblock_type_id = false, $table_name = "b_xml_tree")
	{
		...
		$this->_xml_file = new KCIBlockXMLFile($table_name);
		...
		if ($this->bCatalog)
		{
			...
			while($arBaseUnit = $rsBaseUnit->Fetch()) {
				$this->arMeasureUnits[$arBaseUnit['MEASURE_TITLE']] = $arBaseUnit;
			}
		}
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			while ($arBaseUnit = $rsBaseUnit->Fetch())
			{
				if(strlen($arBaseUnit["ATTRIBUTES"]) > 0)
				{
					...
					if(
						!empty($arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]])
						&& !empty($this->arMeasureUnits[$arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]]]['CODE'])
					)
					{
						$arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]] = $this->arMeasureUnits[$arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]]]['CODE'];
					}
					elseif(
						!empty($info[$this->mess["IBLOCK_XML2_FULL_TITLE"]])
						&& !empty($this->arMeasureUnits[$info[$this->mess["IBLOCK_XML2_FULL_TITLE"]]]['CODE'])
					)
					{
						$arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]] = $this->arMeasureUnits[$info[$this->mess["IBLOCK_XML2_FULL_TITLE"]]]['CODE'];
					}
				}
			}
	...
	function ImportElementPrices($arXMLElement, &$counter, $arParent = false)
	{
		...
		if($arDBElement)
		{
			...
			if(isset($arElement["PRICES"]) && $this->bCatalog)
			{
				...
				$_added=CCatalogProduct::Add($arProduct);
