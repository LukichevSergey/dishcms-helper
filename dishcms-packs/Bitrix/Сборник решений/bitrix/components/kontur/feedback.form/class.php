...
class KonturFeedbackFormComponent extends \CBitrixComponent
{
    ...
    /**
     * $param integer $mode режим выполнения.
     * 0 - стандартное выполнение. Валидация и отправка.
     * 1 - только валидация. Все результаты будут отправлены в поток вывода в JSON формате.
     * 2 - только валидация. При успешной валидации будет возвращено TRUE. При возникновении ошибок
     * результаты будут отправлены в поток вывода в JSON формате.
     * 3 - только валидация. Результат будет возвращен. 
     * 4 - результат будет возвращен.
     *
     ...
     */
    public function send($data, $arParams, $mode=0)
    {
        global $APPLICATION;
        ...
        if(($mode == 4) || ($mode == 3) || (($mode == 2) && $result['success'])) {
            return $result;
        }
