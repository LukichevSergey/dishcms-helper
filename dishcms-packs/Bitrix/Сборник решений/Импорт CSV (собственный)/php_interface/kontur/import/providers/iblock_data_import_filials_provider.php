...
IncludeModuleLangFile(dirname(__FILE__) . '/.last_version/iblock_data_import_filials_provider.php', LANGUAGE_ID);

set_time_limit(0);

class IblockDataImportFilialsProvider
{
    protected static $reverseMapCoords = true;
    
    protected static $urlDataFile = '/upload/import/filials.csv';
    protected static $iblockID = 22;
    protected static $cityPropertyID = 128;
    protected static $partnerTypePropertyID = 125;
    protected static $specializationPropertyID = 127;
    protected static $mapPropertyID = 120;
    protected static $addressPropertyID = 121;
    protected static $columnRelations = [
        0 => 'IE_NAME', // Название (B_IBLOCK_ELEMENT.NAME)
        1 => 'IP_PROP121', // Свойство "Адрес"
        2 => 'IP_PROP120', // Свойство "Карта"
        3 => 'IP_PROP125', // Свойство "Тип партнера"
        4 => 'IP_PROP126', // Свойство "Официальный диллер"
        5 => 'IP_PROP127', // Свойство "Специализация"
        6 => 'IP_PROP122', // Свойство "Email"
        7 => 'IP_PROP123', // Свойство "Телефон"
        8 => 'IP_PROP124', // Свойство "Сайт"
    ];
    
    /**
     * array(csvValue=>propertyValue)
     */
    protected static $partnerType = [
        'ТТ'=>'Магазин',
        'СТО'=>'Сервис'
    ];
    ...
    protected static $specializations = [
        ...
        'Коммерческий транспорт'=>'Грузовая техника'
    ];
    
    public static function init()
    {
        global $URL_DATA_FILE, $IBLOCK_ID;
        
        if(empty($URL_DATA_FILE)) {
            $URL_DATA_FILE = static::$urlDataFile;
        }
    ...
    public static function getDataFileFieldSelected($i, $default=null)
    {
        if(isset(static::$columnRelations[$i])) {
            return static::$columnRelations[$i];
        }
        return $default;
    }
    
    public static function preMapEnum($prop_id, $value)
    {
        if($prop_id == static::$partnerTypePropertyID) { // Свойство "Тип партнера"
            $values = explode(',', $value);
            $value = [];
            foreach($values as $v) {
                $v = trim($v);
                if(isset(static::$partnerType[$v])) {
                    $value[] = static::$partnerType[$v];
                }
                else {
                    $value[] = $v;
                }
        ...
        elseif($prop_id == static::$specializationPropertyID) { // Свойство "Специализация"
            $values = explode(', ', $value);
            $value = [];
            foreach($values as $v) {
                $v = trim($v);
                if(isset(static::$specializations[$v])) {
                    $value[] = static::$specializations[$v];
                }
                else {
                    $value[] = $v;
                }
    ...
    public static function MapString($prop_id, $value)
    {
        if($prop_id == static::$mapPropertyID) { // Свойство "Карта"
            if(!empty($value)) {
                $values = explode(',', $value);
                $coords = [];
                foreach($values as $v) {
    ...
    public static function updateLoadProductArray(&$arLoadProductArray)
    {
        if(isset($arLoadProductArray['PROPERTY_VALUES'])) {
            $props = $arLoadProductArray['PROPERTY_VALUES'];
            
            // получение координат и города
            $geocode = null;
            if(empty($props[static::$mapPropertyID]) && !empty($props[static::$addressPropertyID])) {
                $geocode = $props[static::$addressPropertyID];
            }
            elseif(empty($props[static::$cityPropertyID]) && !empty($props[static::$mapPropertyID])) {
                $geocode = $props[static::$mapPropertyID];
            }
            
            if($geocode) {
                $ch = curl_init('https://geocode-maps.yandex.ru/1.x/?kind=locality&format=json&geocode=' . $geocode);
                ...
                if($response) {
                    ...
                    $found = 0;
                    if(!empty($geoData['response']['GeoObjectCollection']['metaDataProperty']['GeocoderResponseMetaData']['found'])) {
                        $found = (int)$geoData['response']['GeoObjectCollection']['metaDataProperty']['GeocoderResponseMetaData']['found'];
                    }
                    if($found > 0) {
                        if(!empty($geoData['response']['GeoObjectCollection']['featureMember'])) {
                            $break=false;
                            foreach($geoData['response']['GeoObjectCollection']['featureMember'] as $geoObject) {
                                if(!empty($geoObject['GeoObject']['metaDataProperty']['GeocoderMetaData']['Address']['Components'])) {
                                    foreach($geoObject['GeoObject']['metaDataProperty']['GeocoderMetaData']['Address']['Components'] as $addressComponent) {
                                        if(!empty($addressComponent['kind']) && ($addressComponent['kind'] == 'locality') && !empty($addressComponent['name'])) {
                                            $arLoadProductArray['PROPERTY_VALUES'][static::$cityPropertyID] = $addressComponent['name'];
                                            
                                            if(empty($arLoadProductArray['PROPERTY_VALUES'][static::$mapPropertyID]) && !empty($geoObject['GeoObject']['Point']['pos'])) {
                                                $arLoadProductArray['PROPERTY_VALUES'][static::$mapPropertyID] = implode(',', array_reverse(explode(' ', $geoObject['GeoObject']['Point']['pos'])));
                                            }
											elseif(!empty($arLoadProductArray['PROPERTY_VALUES'][static::$mapPropertyID])) {
												$arLoadProductArray['PROPERTY_VALUES'][static::$mapPropertyID]=static::MapString(static::$mapPropertyID, $arLoadProductArray['PROPERTY_VALUES'][static::$mapPropertyID]);
											}
...
IblockDataImportFilialsProvider::init();
...
class CAssocData extends CCSVData
{
	...
	function MapEnum($prop_id, $value)
	{
		...
		$value = IblockDataImportFilialsProvider::preMapEnum($prop_id, $value);
		if (is_array($value))
		{
	...
	if ($STEP > 3)
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			while ($arRes = $csvFile->FetchAssoc())
			{
					...
					foreach ($arRes as $key => $value)
					{
						if (strncmp($key, "IP_PROP", 7) == 0)
						{
							...
							if (is_array($arIBlockProperty[$cur_prop_id]))
							{
								...
								{
								    $value = IblockDataImportFilialsProvider::MapString($cur_prop_id, $value);
								}
				...
				IblockDataImportFilialsProvider::updateLoadProductArray($arLoadProductArray);
...
?>
		<tr>
			...
			<td width="60%">
				<select name="field_<?echo $i ?>">
					<option value=""> - </option>
					<?
        if(!isset(${"field_".$i}) || !${"field_".$i}) {
            ${"field_".$i} = IblockDataImportFilialsProvider::getDataFileFieldSelected($i, $field);
        }
		foreach ($arAvailFields as $ar)
		{
			...
			if (!$bSelected && !isset(${"field_".$i}))
				$bSelected = $ar["code"] == $field; var_dump($bSelected);
?>
