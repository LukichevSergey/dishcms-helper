...
document.addEventListener("DOMContentLoaded",function(){
	...
	var kladrIntervalId=setInterval(function(){if((typeof(KladrJsObj.map)!="undefined") && KladrJsObj.map){konturKladrInit();clearInterval(kladrIntervalId);}else{/*KladrJsObj.nobasemessage();*/}},200);
	...
	//window.BX.locationSelectors[<?=$arParams['LOCATIONS_JS_CONTROL_GLOBAL_ID']?>].setValueByLocationId(<?=$arParams['LOCATIONS_DEFAULT_ID']?>);
});
/*document.addEventListener("DOMContentLoaded",function(){
	//window.oldSoaFormInitialized=false;
	//window.oldSoaFormInitDeliveryId=$(".bx-old-soa-delivery .bx_element [name='DELIVERY_ID']:checked").val();
	//window.oldSoaFormDeliveryHash=false;
	//window.KladrJsObj.submitKladr=function(addr) {console.log(addr);};
	//$("#OLD_SOA_INITIALIZE").val(1); 
	//submitForm();
});*/
</script>
...
<?if(CSaleLocation::isLocationProEnabled()):?>
	...
	<div style="display: none">
		...
		<?$APPLICATION->IncludeComponent(
			...
			".default", 
			array(
//				'ID'=>$arParams['LOCATIONS_DEFAULT_ID'],
//				'PROVIDE_LINK_BY'=>'code',
				'JS_CONTROL_GLOBAL_ID'=>$LOC_STEPS_JS_CONTROL_GLOBAL_ID
			),
			false
		);?>
		<?$APPLICATION->IncludeComponent(
...
//				'ID'=>$arParams['LOCATIONS_DEFAULT_ID'],
//				'PROVIDE_LINK_BY'=>'code',
				'JS_CONTROL_GLOBAL_ID'=>$LOC_SEARCH_JS_CONTROL_GLOBAL_ID
			),
			false
		);?>
	</div>
