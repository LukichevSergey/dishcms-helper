...
<? 
$GLOBALS['OLD_SOA_LOCATIONS_DEFAULT_ID']=$arParams['LOCATIONS_DEFAULT_ID'];
$GLOBALS['OLD_SOA_LOCATIONS_DEFAULT_CODE']=$arParams['LOCATIONS_DEFAULT_CODE'];
$GLOBALS['OLD_SOA_LOCATIONS_JS_CONTROL_GLOBAL_ID']=$arParams['LOCATIONS_JS_CONTROL_GLOBAL_ID'];
...
if (!function_exists("PrintPropsForm"))
{
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		...
		{
			?>
				<div class="order-properties-wrapper">
					<div class="order-properties">
						<?
						foreach ($arSource as $arProperties)
						{
							?>
							<div class="order-property order-property_<?= mb_strtolower($arProperties["TYPE"]) ?>" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
							...
							if ($arProperties["TYPE"] == "CHECKBOX")
							{
								...
								<div class="order-property__label">
									<?=$arProperties["NAME"]?>
								...
								<div class="order-property__input">
							...
							elseif ($arProperties["TYPE"] == "LOCATION")
							{
								...
								if(!$value || !isset($_REQUEST['INITIALIZE']) || (isset($_REQUEST['INITIALIZE']) && ($_REQUEST['INITIALIZE'] == 1))) {
									$value=$GLOBALS['OLD_SOA_LOCATIONS_DEFAULT_ID'];
								}
								?>
								<input type="hidden" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" value="<?=$value?>" />
								<?
								/* ?>
								<div class="order-property__label">
									<?=GetMessage('SOA_TEMPL_LOCATIONS_TITLE')?>
									<? //=$arProperties["NAME"]?>
									<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?>
										<span class="bx_sof_req">*</span>
									<?endif;?>
								...
								<div class="order-property__input">
									...
									<?endif?>
									<?
									if(isset($GLOBALS['OLD_SOA_LOCATIONS_JS_CONTROL_GLOBAL_ID'])) $jsControlId=$GLOBALS['OLD_SOA_LOCATIONS_JS_CONTROL_GLOBAL_ID'];
									else $jsControlId=intval($arProperties["ID"]); 									
									?>
									<?CSaleLocation::proxySaleAjaxLocationsComponent(array(
										...
										//"LOCATION_CODE" => $GLOBALS['OLD_SOA_LOCATIONS_DEFAULT_CODE'],
										//"COUNTRY"=>"0000028023", // Россия - 504
										//"REGION"=>"0000028090", // Сибирь - 1
										//"REGION"=>6,//"0000028095", // Новосибирская область - 6
										//"LOCATION_VALUE"=>"0000949228" // Новосибирск - 216
										//"LOCATION_VALUE"=>216,
										"ORDER_PROPS_ID" => $arProperties["ID"],
										"ONCITYCHANGE" => ($arProperties["IS_LOCATION"] == "Y" || $arProperties["IS_LOCATION4TAX"] == "Y") ? "submitForm();onOldSoaChangeLocation();" : "onOldSoaChangeLocation();",
										"SIZE1" => $arProperties["SIZE1"],
									),
									array(
										"ID" => $value,
										//"CODE" => $GLOBALS['OLD_SOA_LOCATIONS_DEFAULT_CODE'],
