...
$KNTR_IMPORTED_PATH='/home/k/konturdev/tdautoram.konturdev.tmweb.ru/public_html/upload/import_csv/imported';
$KNTR_LOG_FILENAME='/home/k/konturdev/tdautoram.konturdev.tmweb.ru/public_html/upload/import_csv/logs/'.date('Y_m_d_H_i_s').'_log.txt';
$log=function($var, $name) use ($KNTR_LOG_FILENAME) {
	ob_start();
	print_r($var);
	$data=date("d.m.Y H:i:s")."[{$name}]\n";
	$data.=ob_get_contents()."\n";
	ob_end_clean();
	$fp = fopen($KNTR_LOG_FILENAME, 'a+');
	fwrite($fp, $data);
	fclose($fp);
};
$log('start session', 'start');
...
if(!empty($strImportErrorMessage)) {
	$log(pathinfo($_SERVER["DOCUMENT_ROOT"].$DATA_FILE_NAME, PATHINFO_BASENAME), 'Имя файла импорта');
	$log($strImportErrorMessage, '$strImportErrorMessage');
}
else {
	$KNTR_FILE_NAME=$_SERVER["DOCUMENT_ROOT"].$DATA_FILE_NAME;
	$log(pathinfo($KNTR_FILE_NAME, PATHINFO_BASENAME), 'Файл импортирован');
	if (file_exists($KNTR_FILE_NAME) && is_file($KNTR_FILE_NAME)) {
		$NOW_DIRNAME=$KNTR_IMPORTED_PATH . DIRECTORY_SEPARATOR . date('Y_m_d');
		if(is_dir($NOW_DIRNAME))
			$NOW_DIRNAME=$KNTR_IMPORTED_PATH . DIRECTORY_SEPARATOR . date('Y_m_d_H_i_s');
		
		mkdir($NOW_DIRNAME, 0700, true);
		if(copy($KNTR_FILE_NAME, $NOW_DIRNAME.DIRECTORY_SEPARATOR.pathinfo($KNTR_FILE_NAME, PATHINFO_BASENAME))) {
			unlink($KNTR_FILE_NAME);
		}
		else {
			$log($KNTR_FILE_NAME, 'Не удалось скопировать'); 
		}
	}
	// очистка кэша
    ...
    $log('start cache clean', 'cache');
    while ($file = $obCacheCleaner->GetNextFile()) {
        if (is_string($file)) {
            unlink($file);
        }
    }
    $log('end cache clean', 'cache');
}
