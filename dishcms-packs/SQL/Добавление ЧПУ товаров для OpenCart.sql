...
SELECT CONCAT('product_id=', `product_id`), _fs_transliterate_ru(`name`) 
...
SELECT a.keyword, p.product_id, _fs_transliterate_ru(p.name), p.name FROM `oc_product_description` as p 
